...
2. Убедитесь, что вам доступно выставление счетов – в форме **Выставить счет** заполните поле _Сумма_ и нажмите на кнопку. Ниже должна появиться ссылка на счет и кнопка **Скопировать ссылку**.

![invoice-test](/images/p2p-invoice-create-test.png)
...
2. Перейдите на вкладку **API** и нажмите на кнопку **Настроить**.
...
5. Нажмите на кнопку **Дальше**. Пара ключей будет активирована.
...
<aside class="warning">
Доступно выставление счетов только в рублях. Для выставления счетов в тенге используйте <a href="#create">API</a>, <a href="#sdk">SDK</a> или <a href="#cms">решения для CMS</a>.
</aside>
...
---------|--------|---|---------
publicKey | Ваш [публичный ключ для формы переводов](#auth), полученный на сайте p2p.qiwi.com|String|+
...
customFields[paySourcesFilter]|При открытии формы будут отображаться только указанные способы перевода, если они доступны. Возможные значения (можно указать оба, через `,`): <br>`qw` - QIWI Кошелек,<br>`card` - банковская карта.|URL-Encoded String|-
...
<aside class="success">
Рекомендуем рассмотреть возможность использования <a href="/ru/p2p-sdk-guide">SDK</a>.
</aside>
...
Параметр тела запроса|Описание|Тип|Обяз.
---------|--------|---|---------
amount|Информация о сумме счета|Object|+
amount.<br>value| Сумма, на которую выставляется счет, округленная в меньшую сторону до 2 десятичных знаков | Number(6.2)|+
amount.<br>currency| Валюта суммы счета. Возможные значения:<br>`RUB` - рубли,<br>`KZT` - тенге.| Alpha-3 ISO 4217 код |+
...
customFields.<br>paySourcesFilter | Строка с разделителями-запятыми. При [открытии формы](#payurl) будут отображаться только указанные способы перевода (один или несколько), если они доступны. Возможные значения:<br>`qw` - QIWI Кошелек,<br>`card` - банковская карта. |String|-
...
2. Перейдите на вкладку **API** и нажмите на кнопку **Создать пару ключей и настроить**.
...
* Перейдите в личном кабинете в раздел [Форма приема переводов](https://qiwi.com/p2p-admin/transfers/link) и нажмите на кнопку **Настроить**.

   ![alt_text](/images/p2p-sdk-image1.png "Form settings")
...
|Параметр | Описание | Тип | Обязательное|
|---------|----------|----------|---------|
| widgetAlias | Ссылка на форму приема переводов | String | + |
