---
title: P2P-счета 1.0.0

search: true

metatitle: P2P-счета 1.0.0



metadescription: P2P-счета открывает доступ к операциям с выставляемыми счетами. Счет - универсальная заявка на перевод. По умолчанию пользователю доступно несколько способов перевода. В API поддерживаются операции выставления и отмены счетов, а также проверки статуса выполнения операций.
...
###### Последнее обновление: 2020-05-06 | [Редактировать на GitHub](https://github.com/QIWI-API/bill-payments-docs/blob/master/p2p-payments_ru.html.md)

P2P-счета открывает доступ к операциям с выставляемыми счетами. Счет - универсальная заявка на перевод. По умолчанию пользователю доступно несколько способов перевода.
...
* Вы перенаправляете пользователя на [форму](#http) для выставления счета. Или [выставляет счет по API](#create) и перенаправляет пользователя на созданную платежную форму.
...
**Ключи создаются в личном кабинете после  авторизации на [p2p.qiwi.com](https://p2p.qiwi.com) во кладке API.**
...
<aside class="warning">
Уведомление о переводе (callback) отправлялся только по протоколу HTTPS и только на 443 порт.
...
Адрес сервера для уведомлений указывается на <a href="https://p2p.qiwi.com/">p2p.qiwi.com</a> при генерации ключей.
...
Метод позволяет отменить счет по которолму не совершен перевод.
...
customer.phone | Номер телефона пользователя (в международном формате) | URL-закодированная строка|-
customer.email | E-mail пользователя | URL-закодированная строка|-
customer.account | Идентификатор пользователя в вашей системе | URL-закодированная строка |-
...
| allowedPaySources | При открытии формы будут отображаться только указанные способы перевода. (Если они доступны) Возможные значения:<br>`qw` - QIWI Кошелек<br>`card` - банковская карта<br>`mobile` - платеж со счета мобильного телефона<br>`sovest` - карта СОВЕСТЬ<br> | comma separated string
| successUrl | URL для переадресации в случае успешного перевода с баланса QIWI Кошелька. При ином способе перевода переадресация не выполняется. Ссылка должна вести на ваш сайт. | Object |
| lifetime | Дата, до которой счет будет доступен для перевода. Если перевод не будет осуществлен до этой даты, ему присваивается финальный статус EXPIRED и последующая оплата станет невозможна.| String<br>`ГГГГ-ММ-ДДTччмм` |
